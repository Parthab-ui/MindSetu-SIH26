...
function App() {
  ...
  async function runResearchModel() {
    ...
    try {
      ...
      const result = await apiRequest("/api/sih26186/ml/analyze", {
        method: "POST",
        body: JSON.stringify({
          Q29_Total: Number(document.getElementById("q29").value),
          Q12_weapon: Number(document.getElementById("q12").value),
          Q13_feltdie: Number(document.getElementById("q13").value),
          Q23a_cutdowntime: Number(document.getElementById("q23a").value),
          Q23b_Accomplished_less: Number(document.getElementById("q23b").value),
          Q23c_limited_work: Number(document.getElementById("q23c").value),
          Q23d_difficulty_performing: Number(document.getElementById("q23d").value),
  ...
  const nav = sessionId ? (
    <nav className="session-nav">
      <button className={screen === "analysis" ? "active" : ""} onClick={loadDashboard}>Analysis</button>
      <button className={screen === "chat" ? "active" : ""} onClick={() => setScreen("chat")}>AI companion</button>
  ...
  if (screen === "analysis") return (
    <div className="app-shell"><Header darkMode={darkMode} setDarkMode={setDarkMode} />{nav}<main className="page"><div className="page-heading"><span className="eyebrow">04 · WELFARE ANALYSIS</span><h1>Explain the signal, not the person.</h1><p>Deterministic welfare triage is shown separately from the research ML layer.</p></div><div className="metric-grid"><div className="metric-card"><span>Wellness stress</span><strong>{analysis?.wellness_score ?? "—"}</strong></div><div className="metric-card"><span>Duty load</span><strong>{analysis?.workload_score ?? "—"}</strong></div><div className="metric-card"><span>Combined</span><strong>{analysis?.combined_score ?? "—"}</strong></div></div><div className="split-grid"><section className={`card risk-card ${analysis?.risk_level || "low"}`}><span className="eyebrow">WELFARE SIGNAL</span><div className="risk-value">{analysis?.risk_level || "unknown"}</div><p>{analysis?.recommendation || "Complete the workflow to generate a signal."}</p></section><section className="card architecture-card"><span className="eyebrow">SYSTEM CONTRACT</span><h2>Four clear responsibilities</h2><div className="contract"><div><b>LightGBM</b><span>prediction</span></div><div><b>SHAP</b><span>explanation</span></div><div><b>Gemini</b><span>communication</span></div><div><b>Human</b><span>intervention</span></div></div></section></div><div className="card research-card"><div className="card-header"><div><span className="eyebrow">RESEARCH ONLY</span><h2>LightGBM + SHAP</h2><p>Run the validated research model separately so its output is never confused with the operational welfare triage score.</p></div><button className="secondary-button" onClick={checkModel} disabled={loading}>Check model</button></div>{mlResult?.health && <div className="status-line">{mlResult.health.model} ready · threshold {mlResult.health.threshold} · research-only {String(mlResult.health.research_only)}</div>}{mlResult?.error && <div className="error">{mlResult.error}</div>}<div className="ml-grid">{[['q29','Q29 total'],['q12','Q12 weapon exposure'],['q13','Q13 felt life-threat'],['q23a','Q23A reduced work time'],['q23b','Q23B accomplished less'],['q23c','Q23C limited work'],['q23d','Q23D difficulty performing']].map(([id,label]) => <div className="field" key={id}><label>{label}</label><input id={id} type="number" defaultValue={0} /></div>)}</div><button className="primary-button" disabled={loading} onClick={runResearchModel}>Run explainable model →</button>{mlResult?.probability !== undefined && <div className="ml-output"><div className={`signal ${mlResult.signal}`}>{mlResult.signal} · {mlResult.probability.toFixed(3)}</div><p>Clinical diagnosis: {String(mlResult.clinical_diagnosis)}</p><div className="contributors">{(mlResult.contributors || []).slice(0, 5).map((item) => <div className="contributor" key={item.feature}><span>{item.label}</span><b>{item.direction}</b></div>)}</div>{mlResult.supportive_response && <div className="gemini-box"><span className="eyebrow">GEMINI COMMUNICATION</span><p>{mlResult.supportive_response}</p></div>}</div>}</div></main></div>
  );
